...
function uwaveWebClient(uw, options = {}) {
  ...
  const mobile = (req, res) => {
    ...
    fs.createReadStream(_path2.default.join(basePath, 'm.html'), 'utf8').pipe(transform).pipe(res);
  };
  ...
  }).get('/m', mobile).get('/m.html', mobile).use((0, _connectGzipStatic2.default)(basePath));
}
