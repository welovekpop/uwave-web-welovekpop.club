...
var _flashDocumentTitle = require('flash-document-title');

var _flashDocumentTitle2 = _interopRequireDefault(_flashDocumentTitle);
...
function sendChat(text) {
  return function (dispatch, getState) {
    ...
    var message = prepareMessage(state, sender, text, {
      ...
      }), (0, _chatMentions.getAvailableGroupMentions)(hasRole))
    });
    dispatch(message);
  };
}
...
function receive(message) {
  return function (dispatch, getState) {
    ...
    var mentions = [].concat(users.map(function (user) {
      return user.username;
    }), (0, _chatMentions.getAvailableGroupMentions)(function (mention) {
    ...
    if (isMention) {
      ...
      (0, _flashDocumentTitle2.default)('\uD83D\uDCAC ' + sender.username);
    }
  };
}
