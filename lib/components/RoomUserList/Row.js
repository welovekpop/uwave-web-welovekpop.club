"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = void 0;

var _extends2 = _interopRequireDefault(require("@babel/runtime/helpers/builtin/extends"));

var _jsx2 = _interopRequireDefault(require("@babel/runtime/helpers/builtin/jsx"));

var _classnames = _interopRequireDefault(require("classnames"));

var _react = _interopRequireDefault(require("react"));

var _propTypes = _interopRequireDefault(require("prop-types"));

var _compose = _interopRequireDefault(require("recompose/compose"));

var _withProps = _interopRequireDefault(require("recompose/withProps"));

var _List = require("material-ui/List");

var _userCardable = _interopRequireDefault(require("../../utils/userCardable"));

var _Avatar = _interopRequireDefault(require("../Avatar"));

var _Username = _interopRequireDefault(require("../Username"));

var _Votes = _interopRequireDefault(require("./Votes"));

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var enhance = (0, _compose.default)((0, _userCardable.default)(), (0, _withProps.default)(function (props) {
  return {
    onOpenCard: function onOpenCard(event) {
      event.preventDefault();
      props.openUserCard(props.user);
    }
  };
}));

var RoomUserRow = function RoomUserRow(_ref) {
  var className = _ref.className,
      user = _ref.user,
      onOpenCard = _ref.onOpenCard;
  return (0, _jsx2.default)(_List.ListItem, {
    button: true,
    className: (0, _classnames.default)('UserRow', 'UserRow--cardable', className),
    onClick: onOpenCard
  }, void 0, (0, _jsx2.default)(_List.ListItemAvatar, {}, void 0, (0, _jsx2.default)(_Avatar.default, {
    className: "UserRow-avatar",
    user: user
  })), (0, _jsx2.default)(_List.ListItemText, {}, void 0, (0, _jsx2.default)(_Username.default, {
    className: "UserRow-username",
    user: user
  })), _react.default.createElement(_Votes.default, (0, _extends2.default)({
    className: "UserRow-votes"
  }, user.votes)));
};

RoomUserRow.propTypes = process.env.NODE_ENV !== "production" ? {
  className: _propTypes.default.string,
  user: _propTypes.default.object.isRequired,
  onOpenCard: _propTypes.default.func.isRequired
} : {};

var _default = enhance(RoomUserRow); //# sourceMappingURL=Row.js.map


exports.default = _default;
//# sourceMappingURL=Row.js.map
