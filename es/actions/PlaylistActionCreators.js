...
export function flattenPlaylistItem(item) {
  return _extends({}, item.media, item);
}
...
export function loadFilteredPlaylistItems(playlistID) {
  var page = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
...
export function createPlaylist(name) {
  ...
  return post('/playlists', { name: name, description: description, shared: shared }, {
    onStart: function onStart() {
...
export function renamePlaylist(playlistID, name) {
  return put('/playlists/' + playlistID + '/rename', { name: name }, {
    onStart: function onStart() {
      return {
...
export function addMedia(playlist, items) {
  ...
  return post('/playlists/' + playlist._id + '/media', payload, {
    ...
    onComplete: function onComplete(res) {
      return addMediaComplete(playlist._id, res.meta.playlistSize, { afterID: afterID, media: mergeIncludedModels(res).map(flattenPlaylistItem) });
    },
    onError: function onError(error) {
      return {
        type: ADD_MEDIA_COMPLETE,
